...
/*
 	...
 	- peek()	: head의 데이터를 반환, 큐가 비어 있으면 null 반환
 	
 	// 요소 삭제
 	- remove()	: head의 데이터를 삭제하고, 삭제된 값을 반환
 	- poll()	: head의 데이터를 삭제하고, 삭제된 값을 반환, 큐가 비어 있으면 null 반환
 */
public class QueueEx {
	...
	public static void main(String[] args) {
		...
		// queue.remove()	// 요소가 없을 때에도, 삭제를 시도해서, NoSuchElementException 예외 발생
		...
		int remove2 = queue.poll();					// 값 1 삭제
		System.out.println("삭제2 : " + remove2);		
		//	queue.poll();							// 값 2 삭제
		
		// peek()		: head에 있는 값을 반환
		System.out.println("Last item : " + queue.peek()); // 제일 앞에 있는 것
