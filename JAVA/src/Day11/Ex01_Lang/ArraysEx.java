...
public class ArraysEx {
	...
	public static void main(String[] args) {
		...
		// 배열을 정렬(sort) - 내림차순
		Arrays.sort(arr, Collections.reverseOrder()); // collection은 객체만 사용 가능함
		for(int i : arr) {
			System.out.print(i + " ");
		}
		System.out.println();
		...
		// Array.fill(값) : 값으로 모든 배열요소를 채우는 메소드
		int fill[] = new int[10];
		Arrays.fill(fill, 8);
		...
		// Arrays.copyOfRange(배열, 시작index, 끝index);
		// : 배열을 시작index~끝index 까지의 요소를복사하여 배열로 반환
		int copyRange[] = Arrays.copyOfRange(random, 11, 21); // 12~21
