...
public class StreamArrayList {
	...
	public static void main(String[] args) {
		...
		// * 메소드 호출 시, 전달인자로 메소드 레퍼런스로 지정하여 넘겨줄 수 있다.
		// * 메소드를 통해 메소드를 전달하는 람다식을 대체하는 문법이다.
		
		// 이름 순으로 정렬 - 오름차순
		// list.stream().sorted( Comparator.comparing( p -> p.getName() ) ); // 람다식
		list.stream().sorted( Comparator.comparing(Person::getName) ).forEach( p -> System.out.println(p)); // 메소드 레퍼런스
		System.out.println();
		...
		// 이름 순으로 정렬 - 내림차순
		// list.stream().sorted( Comparator.comparing( p -> p.getName() ) ); // 람다식
		list.stream().sorted( Comparator.comparing(Person::getName).reversed() ).forEach( p -> System.out.println(p)); // 메소드 레퍼런스
		System.out.println();
